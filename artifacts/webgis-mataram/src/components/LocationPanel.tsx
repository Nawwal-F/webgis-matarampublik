...
export default function LocationPanel({ feature, onClose }: LocationPanelProps) {
  const { setRouteToFeature, theme } = useApp();
  ...
  const cardBg = isDark ? "bg-gray-800/60 border-gray-700/40" : "bg-gray-50 border-gray-100";
    ...
    <AnimatePresence>
      {feature && info && (
        <motion.div
          ...
          className={`fixed right-0 top-0 h-full w-full max-w-sm backdrop-blur-xl shadow-2xl z-[800] flex flex-col border-l ${panelBg}`}
          style={{ backdropFilter: "blur(20px)" }}
        >
          {/* Header */}
          <div
            className="relative overflow-hidden flex-shrink-0"
            style={{ background: `linear-gradient(135deg, ${info.color}${isDark ? "33" : "22"}, ${info.color}${isDark ? "18" : "11"})` }}
          >
            <div className="absolute inset-0" style={{ background: `radial-gradient(circle at 80% 50%, ${info.color}${isDark ? "25" : "18"}, transparent 70%)` }} />
            <div className="relative px-5 pt-5 pb-4">
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                <button
                  onClick={onClose}
                  className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors flex-shrink-0 mt-0.5 text-sm ${
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              >
                <a
                  ...
                  className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-medium transition-all text-white"
              ...
              >
                <h3 className={`text-xs font-semibold uppercase tracking-wider mb-3 ${isDark ? "text-green-400" : "text-green-700"}`}>📍 Koordinat</h3>
                <div className="grid grid-cols-2 gap-2.5">
                  <div className={`rounded-xl p-3 border ${isDark ? "bg-gray-700/50 border-gray-600/30" : "bg-white border-green-100"}`}>
                    <div className={`text-[10px] mb-1 ${textSecondary}`}>Bujur (Lng)</div>
                    <div className={`text-sm font-mono font-medium ${textPrimary}`}>{lng}°</div>
                  </div>
                  <div className={`rounded-xl p-3 border ${isDark ? "bg-gray-700/50 border-gray-600/30" : "bg-white border-green-100"}`}>
                    <div className={`text-[10px] mb-1 ${textSecondary}`}>Lintang (Lat)</div>
                    <div className={`text-sm font-mono font-medium ${textPrimary}`}>{lat}°</div>
                  </div>
                </div>
              </motion.div>
